...
class iovf:
    ...
    def get_surrounding_indeces(self, update_indeces, window=None):
        if window is None:
            ...
            sys.exit(1)  # Exit if window size is not specified
        ...
        # Iterate over each index in update_indeces
        for j in tqdm(range(np.shape(update_indeces)[0])):
            # Add offsets to each index and ensure no negative values
            for i in range(len(a_)):
                new_update_indeces.append([
                    np.nanmax((update_indeces[j][0] + a_[i], 0)),
                    np.nanmax((update_indeces[j][1] + b_[i], 0)),
                    np.nanmax((update_indeces[j][2] + c_[i], 0))
                ])
        ...
        # Return the unique list of new indices
    ...
    def get_grubbs_chunks(self, noise_cube, window=None,update_indeces=None):
        ...
        if update_indeces is None:
            ...
            for i in tqdm(range(np.shape(update_indeces)[0])):
                ...
                delta = [i0-i1,j0-j1,b0-b1]
                adj_window = [window[g]-delta[g] for g in range(len(window))]
                # if any value in adj_window less than or equal to zero, break and report error
                if any([q <= 0 for q in adj_window]):
                    print(f'window size mismatch :< on pixel\n\t{update_indeces[i]}\n with window size {adj_window}\n delta of {delta}')
                    breakpoint()
                # volume = int(adj_window[0]*adj_window[1]*adj_window[2])
                # vb = np.zeros(volume,dtype=int)
    ...
    def build_vote_cube(self,vbsi,s,window=None,update_indeces=None):
        ...
        if update_indeces is None:
            ...
            for i in tqdm(range(np.shape(update_indeces)[0])):
                ...
                jj = int(np.floor(window[1] / 2))
                bb = int(np.floor(window[2] / 2))
                i0, i1 = max(0, -update_indeces[i][0] + ii), max(0, update_indeces[i][0] + ii + 1 - s[0])
                j0, j1 = max(0, -update_indeces[i][1] + jj), max(0, update_indeces[i][1] + jj + 1 - s[1])
                b0, b1 = max(0, -update_indeces[i][2] + bb), max(0, update_indeces[i][2] + bb + 1 - s[2])
    ...
    def iterative_outlier_filter(self,update_indeces=None,window=None):
        ...
        if update_indeces is None:
            ...
            # check if any update indices are out of the image bounds
            # for u in update_indeces:
            #     if any([q < 0 for q in u]):
            #         print(f'update index out of bounds: {u}')
            #         breakpoint()
            #     elif any([q >= s[i_] for i_, q in enumerate(u)]):
            #         print(f'update index out of bounds: {u}')
            #         exit()
            update_indeces = self.get_surrounding_indeces(update_indeces, window)

            ui_len = np.shape(update_indeces)[0]
            print(f'update indeces shape: {np.shape(update_indeces)}')
            ...
            if ui_len > chunk_thresh:
                ...
                # update_indeces_list = [update_indeces[i:int(np.min(i + ui_chunk_size, ui_len-1))] for i in range(0, ui_len, ui_chunk_size)]
                update_indeces_list = [update_indeces[i:min(i + ui_chunk_size, ui_len)] for i in range(0, ui_len, ui_chunk_size)]
                ...
                for i, ui in enumerate(update_indeces_list):
                    ...
                    grubbs_shape = np.asarray(grubbs_chunks).shape
                    print('\n\tpreparing data for parallel grubbs test')
                    args = [(grubbs_chunks[j],windows[j],identifier[j]) for j in tqdm(range(grubbs_shape[0]))]
                ...
                for u in update_indeces:
                    if any([q < 0 for q in u]):
                        print(f'update index out of bounds: {u}')
                        breakpoint()
                    elif any([q >= s[i] for i, q in enumerate(u)]):
                        print(f'update index out of bounds: {u}')
                        breakpoint()
                grubbs_chunks,windows,identifier = self.get_grubbs_chunks(self.noise_cube, window, update_indeces)
                grubbs_shape = np.asarray(grubbs_chunks).shape
                args = [(grubbs_chunks[k],windows[k],identifier[k]) for k in tqdm(range(grubbs_shape[0]))]
                print('\n\trunning grubbs test')
    ...
    def run_iovf(self, window = None, view_votes = False):
        ...
        while v:
            ...
            if r < 2:
                ...
                update_indeces = tuple(np.array(np.asarray(self.vc>=vt).nonzero()).transpose(1,0)) #np.where(self.vc>=vt).transpose(1,0)
                print(f'vote cube shape: {np.shape(self.vc)}')
                print(f'update indeces shape: {np.shape(update_indeces)}')
                # check if any update indices are out of the image bounds
                for u in update_indeces:
                    if any([q < 0 for q in u]):
                ...
                self.voting_record.append(np.shape(update_indeces)[0])
                v = bool((np.shape(update_indeces)[0])>threshold)
                print(f'condition: {v}')
                ...
                if 'update_indeces' not in locals():
                    update_indeces = tuple(np.array(np.asarray(self.vc>=vt).nonzero()).transpose(1,0))
                previous_update_numbers = np.shape(update_indeces)[0]
                print(f'previous update indeces: {previous_update_numbers}')
                print(f'{update_indeces[:15]}\n...\n{update_indeces[-15:]}')
                print(f'vote cube shape: {np.shape(self.vc)}')
                print(f'vote threshold: {vt}')
                # breakpoint to stop the code here
                # exit()
                self.vc = self.iterative_outlier_filter(update_indeces,window)
                # self.noise_cube = np.where(self.vc>=vt,np.nan,self.noise_cube)
                update_indeces = tuple(np.array(np.asarray(self.vc>=vt).nonzero()).transpose(1,0)) #np.where(self.vc>=vt)
                # ************************************
                toc = timeit.default_timer() - tic
                if toc > 60:
                    print(f'election took {np.round(toc/60,2)} minutes')
                ...
                if r>=14:
                    ...
                    v = bool(np.shape(update_indeces)[0]>threshold and pCheck is False)
                else:
                    v = bool(np.shape(update_indeces)[0]>threshold)
                # ***********************************************************
            ...
            print(f'************\n\t{np.shape(update_indeces)[0]} values to update after {r} rounds of voting\n************')
            print('updating bad values')
            if np.shape(update_indeces)[0]>threshold:
                for i in tqdm(range(np.shape(update_indeces)[0])):
                    ...
                    try:
                        # ****************************
                        # 3d weighted average
                        og_value = self.noise_cube[update_indeces[i][0],
                                                   update_indeces[i][1],
                                                   update_indeces[i][2]]
                        df = copy.deepcopy(self.noise_cube[update_indeces[i][0]-ii+i0:update_indeces[i][0]+ii-i1+1,
                                                           update_indeces[i][1]-jj+j0:update_indeces[i][1]+jj-j1+1,
                                                           update_indeces[i][2]-bb+b0:update_indeces[i][2]+bb-b1+1])
                        ...
                        if weighted_avg == og_value:
                            ...
                            self.noise_cube[update_indeces[i][0],
                                            update_indeces[i][1],
                                            update_indeces[i][2]] = 0
                        else:   
                            self.noise_cube[update_indeces[i][0],
                                            update_indeces[i][1],
                                            update_indeces[i][2]] = weighted_avg
                    except:
    ...
    def run(self, save=False, window=[7,5,5]):
        ...
        self.run_iovf(window=window, view_votes=self.view_votes)
